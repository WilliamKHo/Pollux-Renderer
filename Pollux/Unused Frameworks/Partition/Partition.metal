...
//    // Pad with zero's
//    temp[tid] = tid_absolute < *count ? input[tid_absolute] : 0;
//    uint ai;
//
//    // upsweep
...
//}
//
//kernel void kern_AdjustIndices(device const uint *count               [[  buffer(0)  ]],
...
//    const uint compacted_idx = scan_result_idx + block_sums_idx + sums_y_idx + block_sums_y_idx;
//    
...
//        count_buffer[0] = compacted_idx;
//    }
//}
//
//
//
//kernel void kern_Scatter(device const uint *count               [[  buffer(0)  ]],
//                         device const uint *validation_buffer   [[  buffer(1)  ]],
//                         device const Ray* rays                 [[  buffer(2)  ]],
//                         device       uint *scan_result_buffer  [[  buffer(3)  ]],
//                         device const uint *block_sums          [[  buffer(4)  ]],
...
//
//    // Get scan_result_idx for the offset within the threadgroup
//    const uint compacted_idx   = scan_result_buffer[absolute_idx];
//
//    if (validation_buffer[absolute_idx]) {
//        device_rays[compacted_idx] = rays[absolute_idx];
//    } else {
//        device_rays[absolute_idx - compacted_idx + count_buffer[0]] = rays[absolute_idx];
//    }
//}
//
//
