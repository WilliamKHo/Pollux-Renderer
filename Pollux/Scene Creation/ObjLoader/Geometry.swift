...
extension VertexIndex: CustomStringConvertible {
    public var description: String {
        return "\(String(describing: vIndex))/\(String(describing: nIndex))/\(String(describing: tIndex))"
    }
}
...
public func ==(lhs: Shape, rhs: Shape) -> Bool {
    ...
    if !nestedEquality(lhs: lhs.vertices, rhs.vertices, equal: lengthCheck) ||
       !nestedEquality(lhs: lhs.normals, rhs.normals, equal: lengthCheck) ||
       !nestedEquality(lhs: lhs.textureCoords, rhs.textureCoords, equal: lengthCheck) {
        return false
    }

    let valueCheck: (Vector, Vector) -> Bool = { a, b in
        for i in 0..<a.count {
            if !doubleEquality(a: a[i], b[i]) {
                return false
            }
        }
        return true
    ...
    if !nestedEquality(lhs: lhs.vertices, rhs.vertices, equal: valueCheck) ||
       !nestedEquality(lhs: lhs.normals, rhs.normals, equal: valueCheck) ||
        !nestedEquality(lhs: lhs.textureCoords, rhs.textureCoords, equal: valueCheck) {
            return false
    }

    if !nestedEquality(lhs: lhs.faces, rhs.faces, equal: { $0.count == $1.count }) {
        return false
    }
    ...
    if !nestedEquality(lhs: lhs.faces, rhs.faces, equal: vertexIndexCheck) {
        return false
    }
