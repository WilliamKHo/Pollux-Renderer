...
private func doubleEquality(a: Double, _ b: Double) -> Bool {
    ...
    if a == b { // shortcut for infinities
        ...
        let absA = abs(a)
        let absB = abs(b)
        return diff / min((absA + absB), .leastNormalMagnitude) < 1e-5
    }
}
...
public struct Color {
    ...
    func fuzzyEquals(_ other: Color) -> Bool {
        return doubleEquality(a: self.r, other.r) &&
               doubleEquality(a: self.g, other.g) &&
               doubleEquality(a: self.b, other.b)
    }
}
...
extension OBJMaterial: Equatable {}

public func ==(lhs: OBJMaterial, rhs: OBJMaterial) -> Bool {
